...
void normalizeMetrics(CGeometry& geometry, const CConfig& config,
  ...
  const ScalarType p = SU2_TYPE::GetValue(config.GetAdap_Norm());
  const ScalarType eigmax = 1./(pow(SU2_TYPE::GetValue(config.GetAdap_Hmin()),2.));
  const ScalarType eigmin = 1./(pow(SU2_TYPE::GetValue(config.GetAdap_Hmax()),2.));
  const ScalarType armax2 = pow(SU2_TYPE::GetValue(config.GetAdap_ARmax()), 2.);
  const ScalarType outComplex = ScalarType(config.GetAdap_Complexity());  // Constraint mesh complexity
